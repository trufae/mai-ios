...
enum MCPHTTPClient {
  ...
  static func resetSession(for serverID: UUID) async {
    if let session = await sessions.remove(serverID) {
  ...
  private static func ensureSession(for server: MCPServer, timeout: TimeInterval) async throws
    -> MCPHandshake
  {
    if looksLikeLegacySSEURL(server.baseURL) {
  ...
  private static func perform(
    ...
    guard expectedResponseID != nil else {
      ...
      throw MCPStreamableHTTPError.invalidNotificationStatus(http.statusCode)
    }
  ...
  private static func endpointUsesLegacySSE(
    ...
    configuration.timeoutIntervalForResource = min(timeout, 3)
  ...
  private static func terminate(_ session: MCPSession) async {
    guard let sessionID = session.handshake.sessionID,
      !sessionID.isEmpty,
...
struct MCPServerSentEvent: Equatable, Sendable {
  var event: String?
  var data: String
  var id: String?
  var retryMilliseconds: Int? = nil

  var isLegacyEndpointEvent: Bool {
...
struct MCPServerSentEventParser: Sendable {
  ...
  mutating func consume(line: String) throws -> MCPServerSentEvent? {
  ...
  mutating func finish() throws -> MCPServerSentEvent? {
    dispatch()
  }
...
private actor MCPSessionStore {
  ...
  @discardableResult
  func remove(_ serverID: UUID) -> MCPSession? {
...
private enum MCPStreamableHTTPError: LocalizedError {
  ...
  case invalidNotificationStatus(Int)
  ...
  var errorDescription: String? {
    ...
    case .invalidNotificationStatus(let statusCode):
      return
        "MCP Streamable HTTP returned HTTP \(statusCode) for a JSON-RPC notification; expected HTTP 202."
    case .mismatchedResponseID:
